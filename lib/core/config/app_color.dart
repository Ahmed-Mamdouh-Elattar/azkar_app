import 'package:flutter/material.dart';

class AppColor {
  static const Color darkModeTextColor = Color(0xffE8F5E9);
  static const Color lightModeTextColor = Color(0xff1B5E20);
  static const lightModeButtonColor = Color(0xff2E7D32);
  static const darkModeButtonColor = Color(0xff81C784);
  static const Color darkModeBackgroundColor = Color(0xff0D1B2A);
  static const Color lightModeBackgroundColor = Color(0xffE8F5E9);
}
